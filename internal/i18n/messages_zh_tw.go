...
var ChineseTraditional = Messages{
	...
	OutputStyleHint:    "在 quantara.toml 設定 agent.output_style 即可啟用（下次會話生效）",
	ThemeHeader:        "主題：",
	...
	SlashMCPNone:                 "沒有設定 MCP 伺服器 — 在 quantara.toml 加一個 [[plugins]] 條目",
	...
	ArgEffortMax:    "最高推理",
	ArgThemeCurrent: "當前",
	ArgLanguageAuto: "從 QUANTARA_LANG、REASONIX_LANG 或系統 locale 自動偵測",
	...
	ListHooksNone:       "無生效 hooks — 在 .quantara/settings.json（專案，需信任後）或 ~/.quantara/settings.json（全域）設定",
	ListMcpHeader:       "MCP 伺服器",
	ListMcpNone:         "未連線 MCP 伺服器 — 在 quantara.toml（[[plugins]]）或專案 .mcp.json 中新增",
	...
	MissingKeyIntro:       "quantara.toml 已設定好 — 只差一個 API key 就可以開始。",
	WroteFileFmt:          "已寫入 %s",
	SetupComplete:         "設定完成。",
	SetupCancelled:        "設定已取消。",
	TryHintFmt:            "試試: %s",
	NextHint:              "下一步：設定 API key（執行 `reasonix setup` 或 export OPENAI_API_KEY=...），然後執行 `reasonix run \"你的任務\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳過 quantara.toml 裡的舊 %q 條目（指向 %s）— 請手動從 [[providers]] 裡刪除",
