...
var Chinese = Messages{
	...
	SetKeyHint:      "设置好 API key 后运行 `quantara`。",
	ConfigLabel:     "配置",
	ModelsLabel:     "模型",
	...
	InitHint:       "项目记忆（AGENTS.md）在会话内由模型生成：运行 `quantara`，然后 `/init` —— 模型会分析代码库并写入。配置请用 `quantara setup`。",
	StepSetKeyHint: "运行 `quantara setup`，或 export OPENAI_API_KEY=…",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
	HelpFooter:     "quantara help · 查看全部命令",

	ChatTip:           "对话上下文将跨轮保留。输入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "没有可恢复的会话 — 用 `quantara` 开一个新的",
	...
	OutputStyleHint:    "在 quantara.toml 设置 agent.output_style 即可启用（下次会话生效）",
	ThemeHeader:        "主题：",
	...
	SlashMCPNone:                 "没有配置 MCP 服务器 — 在 quantara.toml 加一个 [[plugins]] 条目",
	...
	ArgEffortMax:    "最高推理",
	ArgThemeCurrent: "当前",
	ArgLanguageAuto: "从 QUANTARA_LANG、REASONIX_LANG 或系统 locale 自动检测",
	...
	ListHooksNone:       "无生效 hooks — 在 .quantara/settings.json（项目，需信任后）或 ~/.quantara/settings.json（全局）配置",
	ListMcpHeader:       "MCP 服务器",
	ListMcpNone:         "未连接 MCP 服务器 — 在 quantara.toml（[[plugins]]）或项目 .mcp.json 中添加",
	...
	MissingKeyIntro:       "quantara.toml 已配置好 — 只差一个 API key 就可以开始。",
	WroteFileFmt:          "已写入 %s",
	SetupComplete:         "设置完成。",
	SetupCancelled:        "设置已取消。",
	TryHintFmt:            "试试: %s",
	NextHint:              "下一步：设置 API key（运行 `reasonix setup` 或 export OPENAI_API_KEY=...），然后运行 `reasonix run \"你的任务\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳过 quantara.toml 里的旧 %q 条目（指向 %s）— 请手动从 [[providers]] 里删除",
	...
	ProviderErrAuth:                "认证失败 (HTTP 401)：未读到 API key（缺失或未设置）。请在 .env 中配置密钥，或运行 `quantara setup`。",
	ProviderErrAuthRejected:        "认证失败 (HTTP 401)：服务端拒绝了你的 API key。可能是 key 错误或已过期，也可能是服务端出现瞬时鉴权/额度问题——已退避重试仍失败。请稍后再试，或检查 .env 中的密钥 / 运行 `quantara setup`。",
	...
	UsageBody: `quantara — 由配置和插件驱动的 coding agent（多模型）

用法：
  quantara [--model NAME] [-c|--continue] [--resume] [--yolo] [--dir PATH]   交互式会话（多轮；-c 恢复最近一次，--resume 选择一个）
  quantara run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   执行单次任务后退出
  quantara review [--base BRANCH] [--commit SHA] [--model NAME]  AI 代码审查（基于本地 diff）
  quantara serve [--model NAME] [--addr HOST:PORT]      通过 HTTP+SSE 提供会话（浏览器客户端在 /）
  quantara acp [--model NAME]                           通过 stdio 提供 Agent Client Protocol（也可用：quantara --acp）
  quantara setup [path]                                 交互式配置向导；生成 quantara.toml（及 .env）
  quantara config auto-plan [off|on]                    配置自动计划模式
  quantara config reasoning-language [auto|zh|en]        配置可见思考语言
  quantara mcp <add|remove|list|import>                 管理 quantara.toml 里的 MCP 服务器
  quantara init                                         查看如何生成项目记忆（AGENTS.md）
  quantara doctor [--json]                              输出脱敏的本地诊断信息
  quantara bot start|doctor|weixin-login                多渠道 IM bot 网关
  quantara upgrade [--check] [--force]                   自更新到最新版本（也可用：quantara update）
  quantara version
  quantara help
...
示例：
  quantara
  quantara --continue
  quantara run "把 main.go 里的 TODO 实现掉"
  quantara run --model mimo-pro "给这个函数补单元测试"
  echo "解释这段代码" | quantara run

配置：
  优先级：flag > ./quantara.toml > ~/.quantara/config.toml > 内置默认值
  密钥通过 api_key_env 从环境变量注入（如 OPENAI_API_KEY）。
  运行 'quantara setup' 生成配置；详见 docs/SPEC.md。
`,
}
