...
var Chinese = Messages{
	...
	StepSetKeyHint: "运行 `reasonix setup`，或 export OPENAI_API_KEY=…",
	StepChatDesc:   "交互式会话",
	StepRunDesc:    "执行单次任务",
...
用法：
  ...
  reasonix setup [path]                                 交互式配置向导；生成 quantara.toml（及 .env）
  reasonix config auto-plan [off|on]                    配置自动计划模式
  reasonix config reasoning-language [auto|zh|en]        配置可见思考语言
  reasonix mcp <add|remove|list|import>                 管理 quantara.toml 里的 MCP 服务器
