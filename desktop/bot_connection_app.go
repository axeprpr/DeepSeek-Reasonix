...
func (a *App) TestBotConnection(id, target string) (BotConnectionDiagnostic, error) {
	...
	case "feishu":
		...
		result, err = feishu.SendText(ctx, feishuCfg, target, "Quantara bot 测试消息：连接和发送链路可用。")
	case "weixin":
		...
		result, err = weixin.SendText(ctx, weixinCfg, target, "Quantara bot 测试消息：连接和发送链路可用。")
	}
	if err != nil {
